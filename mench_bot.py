...
async def on_dice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if len(moves) > 1:
        kb = InlineKeyboardMarkup([[InlineKeyboardButton(f"مهره {i+1}", callback_data=f"move:{i}:{value}:{dice_message_id}")]
                                     for i in moves])
        png = render_game_png(game, choice_color=current_color, choice_indices=moves)
        media = InputMediaPhoto(media=png, caption="کدوم مهره رو حرکت بدم؟ (عدد روی مهره‌ها رو ببین)")
        try:
            await context.bot.edit_message_media(chat_id=chat_id, message_id=board_message_id,
                                                    media=media, reply_markup=kb)
        except Exception:
            log.exception("edit_message_media failed for piece choice")
            await context.bot.send_photo(chat_id=chat_id, photo=png,
                                           caption="کدوم مهره رو حرکت بدم؟", reply_markup=kb)
        return
...
async def on_piece_choice(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    _, piece_idx, value, dice_message_id = query.data.split(":")
    piece_idx, value = int(piece_idx), int(value)
    dice_message_id = int(dice_message_id)
    await query.answer()
